...
func getIsuID(c echo.Context) error {
	...
	cached, ok := isucache.Load(jiaIsuUUID)
...
func getTrend(c echo.Context) error {
	...
	cache, ok := trendCache.Load(now)
	var res = make([]TrendResponse, 0, 200)

	if ok {
		res = cache.([]TrendResponse)
	} else {
		characterList := []Isu{}
